          ...
          <div className="h-64 w-full" style={{ minHeight: "250px" }}>
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={workload} layout="vertical" margin={{ left: 40 }}>
                ...
                />
                <Bar dataKey="completed" stackId="a" fill="#00C49F" name="Completed" radius={[0, 4, 4, 0]} />
                <Bar dataKey="inProgress" stackId="a" fill="#FFBB28" name="In Progress" />
                <Bar dataKey="todo" stackId="a" fill="#0088FE" name="Todo" radius={[4, 0, 0, 4]} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </motion.div>
          ...
          <table className="w-full text-sm text-left">
            <thead className="bg-gray-50 text-gray-500 uppercase text-xs">
              <tr>
                ...
                <th className="px-6 py-3 text-center text-red-600">Overdue Stats</th>
              </tr>
            </thead>
                 ...
                 <tr key={user.userId} className="hover:bg-gray-50 transition-colors">
                   ...
                   <td className="px-6 py-4 text-center">
                     <span className={`px-2 py-1 rounded-full text-xs font-semibold ${
                       ...
                       user.completionRate >= 50 ? "bg-yellow-100 text-yellow-700" :
                       "bg-red-100 text-red-700"
                     }`}>
                       {Math.round(user.completionRate)}%
                     </span>
                   </td>
                   <td className="px-6 py-4 text-center text-gray-600">{user.total}</td>
                   <td className="px-6 py-4 text-center text-gray-600">{user.completed}</td>
                   <td className="px-6 py-4 text-center text-gray-600">{user.inProgress}</td>
...
const StatCard = ({ title, value, color, subtext }: { title: string; value: string | number; color: string; subtext: string }) => (
  ...
  >
    ...
    <div className={`mt-2 text-3xl font-bold ${color.split(' ')[1]}`}>{value}</div>
